...
Active Directory keeps its DNS data in the directory, so SAMADCON reads and
...
from samadcon.ad import dnsrecords, values
...
def advance_zone_serial(conn: DirectoryConnection, zone_dn: str) -> int:
    ...
    writes with the serial it got back — SAMADCON follows suit, so that records
