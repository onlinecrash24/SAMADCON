...
def test_dc_hosts_from_a_comma_separated_env_var(monkeypatch: pytest.MonkeyPatch):
    settings = _settings(
        monkeypatch,
        SAMCON_REALM="example.lan",
        SAMCON_DC_HOSTS="dc1.example.lan,dc2.example.lan",
    )
...
def test_dev_mode_does_not_disable_tls_verification(monkeypatch: pytest.MonkeyPatch):
    ...
    settings = _settings(monkeypatch, SAMCON_REALM="example.lan", SAMCON_DEV_MODE="1")
    assert settings.tls_verify_peer == "ca_and_name"


def test_insecure_flag_disables_verification(monkeypatch: pytest.MonkeyPatch):
    settings = _settings(monkeypatch, SAMCON_REALM="example.lan", SAMCON_LDAP_INSECURE="1")
    assert settings.tls_verify_peer == "no_check"


@pytest.mark.parametrize("variable", ["SAMCON_LDAP_CA_FILE", "SAMCON_SERVERS_FILE"])
def test_empty_path_variables_are_treated_as_unset(
    ...
    Without normalisation that becomes Path("."), and SAMCON would hand the
    working directory to Samba as a CA bundle.
    """
    settings = _settings(monkeypatch, SAMCON_REALM="example.lan", **{variable: ""})
...
def test_a_real_path_still_works(monkeypatch: pytest.MonkeyPatch):
    settings = _settings(
        monkeypatch, SAMCON_REALM="example.lan", SAMCON_LDAP_CA_FILE="/etc/samcon/ca/ca.pem"
    )
...
def test_whitespace_only_path_is_unset(monkeypatch: pytest.MonkeyPatch):
    settings = _settings(monkeypatch, SAMCON_REALM="example.lan", SAMCON_LDAP_CA_FILE="   ")
    assert settings.ldap_ca_file is None


def test_log_level_is_uppercased(monkeypatch: pytest.MonkeyPatch):
    assert _settings(monkeypatch, SAMCON_REALM="example.lan", SAMCON_LOG_LEVEL="debug").log_level == "DEBUG"
