...
Everything is driven by SAMADCON_* environment variables; the entrypoint derives
the Samba configuration from the same values before the application starts.

A realm is no longer required here: administrators can point SAMADCON at a
...
class ServerProfile(BaseModel):
    ...
    Loaded from the JSON file named by SAMADCON_SERVERS_FILE::
        ...
        [
          {
            ...
            "realm": "EXAMPLE.LAN",
            "ca_file": "/etc/samadcon/ca/example.pem"
          }
        ]
...
class Settings(BaseSettings):
    ...
    # --- Sessions ---------------------------------------------------------
    ccache_dir: Path = Path("/dev/shm/samadcon-ccache")
    session_idle_minutes: int = 60
    login_max_attempts: int = 5
    login_lockout_minutes: int = 5
    cookie_name: str = "samadcon_session"
    cookie_secure: bool = True

    # --- Samba ------------------------------------------------------------
    smb_conf: Path = Path("/etc/samadcon/smb.conf")
    krb5_config: Path = Path("/etc/samadcon/krb5.conf")
    samba_log_level: int = 0

    # --- Runtime ----------------------------------------------------------
    log_level: str = "INFO"
    audit_file: Path = Path("/var/log/samadcon/audit.jsonl")
    ...
    def tls_verify_peer(self) -> str:
        ...
        Only an explicit SAMADCON_LDAP_INSECURE=1 turns validation off — dev_mode
