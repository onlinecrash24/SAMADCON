...
class Catalogue:
    ...
    def supported_text(self, policy: Policy) -> str | None:
        """What a policy needs, as text where the definition is installed.

        The reference is kept otherwise: it names a template that may be
        added later, and half an answer beats none.
        """
        if not policy.supported_on:
            return None
        return self.supported_on.get(policy.supported_on, policy.supported_on)
