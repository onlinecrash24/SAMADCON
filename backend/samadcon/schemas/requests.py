...
class CreateDnsRecordRequest(DnsRecordBase):
    # Type-specific; validated against the record type in samadcon.ad.dnsrecords.
    data: dict[str, Any]
...
class CreateSubnetRequest(StrictModel):
    # Validated as a network prefix in samadcon.ad.sites; a pattern here would
...
class ApplyPolicyRequest(StrictModel):
    ...
    # Keyed by element id; validated against the template in samadcon.gpo.admx.
...
class SetSecurityValueRequest(StrictModel):
    ...
    *value* is a string for a plain setting and a list of SIDs for a user
    ...
    # section: samadcon.gpo.security.check_safe explains why, and enforces it
...
class AddAceRequest(StrictModel):
    ...
    mask: int = Field(gt=0, le=0xFFFFFFFF, description="Access mask; see samadcon.ad.rights")
    deny: bool = False
