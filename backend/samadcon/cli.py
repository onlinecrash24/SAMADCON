"""samadconctl — diagnostics for the container.

Runs inside the running container (`docker compose exec samadcon samadconctl ...`)
and answers the questions that come up when SAMADCON cannot reach a domain:
...
def cmd_discover(args: argparse.Namespace) -> int:
    ...
    if target is None:
        print(
            "No default domain is configured (SAMADCON_REALM is empty).\n"
            "Use `samadconctl probe <address>` to inspect a specific server.",
            file=sys.stderr,
        )
        return 1
    ...
    source = "SAMADCON_DC_HOSTS" if settings.dc_hosts else "DNS SRV records"
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    from samadcon.ad.connection import connect
    ...
    ccache = Path(settings.ccache_dir) / "samadconctl-check"
...
def cmd_sysvol(args: argparse.Namespace) -> int:
    ...
    Group policy is the only part of SAMADCON that needs SMB as well as LDAP, and
    ...
    password = args.password or getpass.getpass(f"Password for {username}: ")
    principal = kerberos.parse_principal(username, target.realm)
    ccache = Path(settings.ccache_dir) / "samadconctl-sysvol"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="samadconctl", description="SAMADCON diagnostics")
    parser.add_argument("--version", action="version", version=f"SAMADCON {__version__}")
    sub = parser.add_subparsers(dest="command", required=True)

    sub.add_parser("config", help="show the effective configuration").set_defaults(
        func=cmd_config
    )
    sub.add_parser("discover", help="list the domain controllers SAMADCON would use").set_defaults(
        func=cmd_discover
    )
