...
from samadcon.api.v1 import router as api_router
...
logger = logging.getLogger("samadcon")
...
STATIC_DIR = Path("/srv/samadcon/www")
...
async def lifespan(app: FastAPI):
    ...
    if settings.realm:
        logger.info(
            "SAMADCON %s starting (default realm=%s, dc=%s)",
            __version__,
            settings.realm,
        ...
        logger.info(
            "SAMADCON %s starting (no default realm; %d configured domain(s), "
            "custom addresses %s)",
            __version__,
            len(profiles),
    ...
    if settings.ldap_insecure:
        logger.warning(
            "SAMADCON_LDAP_INSECURE=1 — LDAPS certificates are not validated. "
            "Do not run this way in production."
        )
    if settings.dev_mode:
        logger.warning("SAMADCON_DEV_MODE=1 — development conveniences are enabled.")
    ...
    try:
        ...
        logger.info("SAMADCON stopped")


app = FastAPI(
    title="SAMADCON — Samba AD Console",
    description=(
...
@app.exception_handler(SamadconError)
async def handle_samadcon_error(request: Request, exc: SamadconError) -> JSONResponse:
