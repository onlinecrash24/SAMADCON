...
:mod:`samadcon.core.executor`) — an ldb handle must not be shared between
threads. Nothing here may be awaited directly from a router; go through
:func:`samadcon.ad.session_directory.directory_for` instead.
"""
...
from samadcon.core.errors import (
    InvalidRequest,
    SamadconError,
    UpstreamUnavailable,
    translate,
...
class DirectoryConnection:
    ...
    def __init__(
        ...
        self.target = target
        self.ccache = ccache
        # Filled in lazily by samadcon.gpo.sysvol; one SMB session per directory
...
def _build_loadparm(settings: Settings, target: ConnectionTarget, transport: str) -> Any:
    ...
    # SamDB constructor — see samadcon.auth.kerberos.apply_transport_settings.
    from samadcon.auth.kerberos import load_loadparm
    ...
    if all(_is_address(host) for host in candidates):
        ...
        raise UpstreamUnavailable(
            ...
            hint=(
                ...
                "address. SAMADCON reads the name from the DC itself, so this "
