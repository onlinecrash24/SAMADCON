...
from samcon.ad.connection import SCOPE_SUBTREE, DirectoryConnection
from samcon.ad.directory import summarize
from samcon.core.errors import Conflict, InvalidRequest, NotFound, PermissionDenied
...
def update_computer(
    ...
    *,
    ...
    from samcon.ad.users import _set_uac
...
def reset_computer_account(conn: DirectoryConnection, dn: str) -> None:
    ...
    Account" does.
    """
    from samcon.ad.users import set_password
...
def read_laps_password(conn: DirectoryConnection, dn: str) -> dict[str, Any]:
    ...
    if values.as_bytes(entry, "msLAPS-EncryptedPassword") is not None:
        raise InvalidRequest(
            ...
            code="laps_encrypted",
            hint="Windows LAPS encryption is not supported by SAMCON yet.",
        )
