...
SAMCON used to be configured for a single domain at container start. It now
...
1. a server the user typed into the sign-in form (realm discovered from the
   rootDSE — see :mod:`samcon.ad.discovery`),
    ...
    def __post_init__(self) -> None:
        if not self.realm:
            raise InvalidRequest(
                "No domain was given.",
                code="missing_realm",
                hint="Enter a server address, or configure SAMCON_REALM.",
            )
