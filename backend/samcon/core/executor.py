...
from samcon.core.errors import OperationTimeout, SamconError, translate
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # here it is a SamconError either way. What separates them is that
            # a translated one still carries what it wrapped.
            ours = isinstance(exc, SamconError) and exc.__cause__ is None
            logger.log(
        ...
        if worker is None:
            if len(self._workers) >= self.max_sessions:
                raise SamconError(
                    ...
                    status_code=503,
                    hint="Wait for other sessions to expire or raise SAMCON_MAX_SESSIONS.",
                )
