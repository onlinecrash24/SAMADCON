...
which produces a :class:`SamconError` carrying
...
class AuthenticationError(SamconError):
    status_code = 401
    code = "authentication_failed"


class SessionExpired(SamconError):
    status_code = 401
    code = "session_expired"


class PermissionDenied(SamconError):
    status_code = 403
    code = "insufficient_access"


class NotFound(SamconError):
    status_code = 404
    code = "not_found"


class Conflict(SamconError):
    status_code = 409
    code = "conflict"


class InvalidRequest(SamconError):
    status_code = 400
    code = "invalid_request"


class ConstraintViolation(SamconError):
    status_code = 400
    code = "constraint_violation"


class UpstreamUnavailable(SamconError):
    status_code = 502
    code = "dc_unavailable"


class OperationTimeout(SamconError):
    status_code = 504
    code = "timeout"
...
_LDAP_ERRORS: dict[int, tuple[type[SamconError], str, str, str | None]] = {
    ...
    LDB_ERR_SIZE_LIMIT_EXCEEDED: (
        ...
        "Add a filter; SAMCON pages results but the server limit still applies.",
    ),
...
_AD_BIND_SUBCODES: dict[str, tuple[str, str, str | None]] = {
    ...
    "531": (
        ...
        "SAMCON authenticates from the container — check the account's logon workstations.",
    ),
    "532": (
        "password_expired",
...
_NT_STATUS: dict[str, tuple[type[SamconError], str, str, str | None]] = {
    ...
    "NT_STATUS_TIME_DIFFERENCE_AT_DC": (
        AuthenticationError,
        "clock_skew",
        "The clocks of SAMCON and the domain controller differ too much.",
...
_KRB_PATTERNS: list[tuple[re.Pattern[str], type[SamconError], str, str, str | None]] = [
    (
    ...
    (
        ...
        "Check SAMCON_DC_HOSTS, DNS SRV records and that port 88 is open.",
    ),
    (
    ...
    (
        ...
        "Point SAMCON_LDAP_CA_FILE at the CA that signed it.",
    ),
]
...
def translate(exc: BaseException) -> SamconError:
    """Turn any exception from the Samba layer into a :class:`SamconError`."""
    if isinstance(exc, SamconError):
        return exc
    ...
    return SamconError("An unexpected error occurred.", detail=text)
...
def _translate_ldap(exc: BaseException, text: str) -> SamconError | None:
    ...
    if code_num is None:
        return SamconError("The directory server reported an error.", detail=detail)

    cls, code, message, hint = _LDAP_ERRORS.get(
        code_num,
        (SamconError, "ldap_error", "The directory server reported an error.", None),
    )
