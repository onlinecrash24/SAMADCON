...
administrator who signed in — SAMCON itself never needs a privileged account.
...
from samcon.core.errors import AuthenticationError, SamconError, translate
...
def load_loadparm(settings: Settings, target: ConnectionTarget, *, transport: str = "ldap"):
    ...
    # SAMCON_SAMBA_LOG_LEVEL takes effect without rebuilding the container.
...
def _acquire_via_kinit(
    ...
    """
    from samcon.auth.krb5conf import get_krb5_configuration

    env = dict(os.environ)
    env["KRB5CCNAME"] = ccache_url(ccache)
    # The generated configuration knows every realm SAMCON has been pointed
...
def acquire_ticket(
    ...
    Raises a :class:`~samcon.core.errors.SamconError` subclass on failure; the
