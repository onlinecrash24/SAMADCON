...
| `0.6.1` | One release, and it never changes. **Pin this where an upgrade should be a decision.** |
| `0.6` | The newest release of that minor series. |
...
the template's logon name ends in the new one. It does not take the description, office,
telephone, mail, title or street. The password is generated by default, to the domain's policy
and never containing the account's names, set to be changed at first logon, and shown once
after the account exists — it is stored nowhere and left out of the audit log. A group the
signed-in account may not write to is reported, and the new account stays.
